        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-[10px] font-extrabold uppercase tracking-[0.2em]">
              ...
              Ma'muriy boshqaruv paneli
            </div>
            <h2 className="mt-6 text-4xl font-black tracking-tight lg:text-5xl">Tizim nazorati va foydalanuvchi boshqaruvi</h2>
            <p className="mt-4 max-w-2xl text-sm font-medium leading-7 text-white/70">
              Kurslar, foydalanuvchilar va retake jarayonlari bo'yicha asosiy ko'rsatkichlar shu yerda jamlangan.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link to="/admin/teachers/create" className="inline-flex items-center gap-2 rounded-2xl bg-primary px-5 py-3 text-sm font-bold text-white">
              <UserPlus size={16} />
              O'qituvchi qo'shish
            </Link>
            <Link to="/admin/students/create" className="inline-flex items-center gap-2 rounded-2xl border border-white/15 bg-white/5 px-5 py-3 text-sm font-bold text-white">
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)]">
        <div className="card overflow-hidden">
          <div className="border-b border-border/60 px-6 py-5 lg:px-8">
            <h3 className="text-xl font-black tracking-tight text-text-primary">Oxirgi yaratilgan kurslar</h3>
          </div>
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {data?.recent_courses.length ? data.recent_courses.map((course) => (
                    <tr key={course.id} className="hover:bg-slate-50/50">
                      <td className="px-6 py-5">
                        <Link to={course.spa_path} className="font-bold text-text-primary hover:text-primary">{course.title}</Link>
                    ...
                    <tr>
                      <td colSpan={4} className="px-6 py-16 text-center text-sm font-medium text-text-secondary">Kurslar mavjud emas.</td>
                    </tr>
                  )}
                </tbody>
              </table>
