...
import {
  Users, ClipboardList, BookOpen, ArrowLeft,
  Plus, Zap, Loader, X, CheckCircle2, Clock,
} from 'lucide-react';
import {
...
import type { PendingStudent } from '@/src/api/retake';
...
interface StudentGroupInfo {
  ...
  control_types_needed: string[];
  assessment_status: Record<string, { id: number; status: string } | null>;
  all_scheduled: boolean;
}
...
export default function RetakeGroupDetailPage() {
  ...
  const [studentGroupsForAssessment, setStudentGroupsForAssessment] = useState<StudentGroupInfo[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setStudentGroupsForAssessment((sg.student_groups || []) as StudentGroupInfo[]);
    } catch {
      /* ignore */
    } finally {
      setIsLoading(false);
          ...
          <div className="text-right shrink-0">
            <p className="text-sm font-bold text-text-secondary">
              ...
              {data.group.teacher?.full_name ?? (
                <span className="text-warning">Tayinlanmagan</span>
              )}
            </p>
...
function MembersTab({
  ...
  const loadPending = useCallback(async () => {
    try {
      const d = await fetchGroupPendingStudents(groupId);
        ...
        <div className="card border-2 border-amber-200 bg-amber-50/50 p-5 space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="font-black text-amber-900 text-base">
                {pendingCount} ta talaba guruhga biriktirilmagan
              </h3>
              <p className="text-xs text-amber-700 mt-0.5">
                Quyidagi talabalar HEMIS guruhlari bo'yicha ko'rsatilgan.
                "Barcha talabalarni biriktir" tugmasini bosing.
              </p>
            </div>
            <button
          ...
          <div className="grid gap-2">
            {pendingByGroup.map(pg => (
              <div
                key={pg.hemis_group}
                className="rounded-xl bg-white border border-amber-200 px-4 py-3"
              >
                ...
                <p className="text-xs text-amber-700">
                  {pg.students.map(s => s.student_name).join(' • ')}
                </p>
              </div>
            ))}
          </div>
      ...
      {studentGroups.length === 0 ? (
        pendingCount === 0 && (
          <div className="card p-12 text-center text-text-muted text-sm">
            Hali hech bir talaba biriktirilmagan
          </div>
        )
      ) : (
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <h3 className="font-black text-text-primary text-xs uppercase tracking-wider">
              Imtihon guruhlari ({studentGroups.length} ta)
            </h3>
          </div>
              ...
              <div className="border-t border-border overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {sg.members.map(m => (
                      <tr key={m.membership_id} className="hover:bg-slate-50/50 group/row">
                        ...
                        <td className="px-4 py-3 text-right">
                          <button
                            ...
                            className="opacity-0 group-hover/row:opacity-100 text-danger hover:bg-danger/10 rounded-lg p-1.5 transition-all"
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if ((res as any).success) {
        ...
        setError((res as any).error || 'Xatolik');
      }
    } catch {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-xs font-black uppercase tracking-wider text-text-muted">
                <tr>
                  ...
                  {CONTROL_TYPES.map(ct => (
                    <th key={ct.value} className="px-3 py-3 text-center min-w-[100px]">{ct.label}</th>
                  ))}
              ...
              <tbody className="divide-y divide-border">
                {studentGroups.map(sg => {
                  const covered = CONTROL_TYPES.every(ct => assessmentMap[sg.group_name]?.[ct.value]);
                  return (
                    <tr key={sg.group_name} className="hover:bg-slate-50/40">
                      <td className="px-5 py-3.5">
                        <p className="font-black text-text-primary">{sg.group_name}</p>
                        <p className="text-xs text-text-muted">{sg.student_count} talaba &bull; {sg.faculty_name}</p>
                      </td>
                      {CONTROL_TYPES.map(ct => {
                            ...
                            {assessment ? (
                              <div className="flex flex-col items-center gap-0.5">
                                <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${
                                  assessment.status === 'open' ? 'bg-emerald-50 text-emerald-700'
                                  : assessment.status === 'closed' ? 'bg-slate-100 text-slate-500'
      ...
      {showForm && (
        <div className="card p-6 border-2 border-primary/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="label-micro mb-1.5 block">Imtihon guruhi *</label>
              <select
            ...
            <div>
              <label className="label-micro mb-1.5 block">Nazorat turi *</label>
              <select
            ...
            <div>
              <label className="label-micro mb-1.5 block">Sana va vaqt *</label>
              <input
                type="datetime-local"
            ...
            <div>
              <label className="label-micro mb-1.5 block">Xona</label>
              <input
                type="text"
            ...
            <div>
              <label className="label-micro mb-1.5 block">O'qituvchi</label>
              <select
            ...
            <div>
              <label className="label-micro mb-1.5 block">Para raqami</label>
              <input
                type="number"
                min={1}
                max={6}
              ...
              <div key={a.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-slate-50">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${
                    a.status === 'open' ? 'bg-emerald-50 text-emerald-700'
                    : a.status === 'closed' ? 'bg-slate-100 text-slate-500'
                    : 'bg-amber-50 text-amber-700'
                  }`}>
                    {a.status_label}
                  </span>
      ...
      {sheets.map(a => {
        ...
        return (
          <div key={sheet.id} className="card px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${
                sheet.status === 'open' ? 'bg-emerald-50 text-emerald-700'
                : sheet.status === 'submitted' ? 'bg-blue-50 text-blue-700'
                : sheet.status === 'locked' ? 'bg-slate-100 text-slate-500'
                : 'bg-amber-50 text-amber-700'
              }`}>
                {sheet.status_label}
              </span>
